...
def load_and_train_model():
    netflix_df = pd.read_csv(r'Data_set/netflix_users_bad_model.csv' )
    netflix_df = netflix_df.dropna()

#จัดการคอลัมน์ 'Name'
    netflix_df = netflix_df[~netflix_df['Name'].str.isnumeric()]  # ลบแถวที่ Name เป็นตัวเลข
    netflix_df = netflix_df.dropna(subset=['Name'])
    
    
#จัดการคอลัมน์ 'Country'
    netflix_df = netflix_df[netflix_df['Country'].str.isalpha()]  # ลบแถวที่ Country เป็นตัวเลข
...
#จัดการคอลัมน์ 'Age'
    ...
    netflix_df = netflix_df.dropna(subset=['Age'])
...
#จัดการคอลัมน์ Favorite_Genre ที่ผิดปกติ
    netflix_df = netflix_df[~netflix_df['Favorite_Genre'].str.isnumeric()]
